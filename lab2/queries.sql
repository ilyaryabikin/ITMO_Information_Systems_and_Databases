...
*/
SELECT t.НАИМЕНОВАНИЕ, v.ИД FROM Н_ТИПЫ_ВЕДОМОСТЕЙ as t
LEFT JOIN Н_ВЕДОМОСТИ as v ON t.ИД = v.ТВ_ИД
...
INNER JOIN Н_УЧЕНИКИ as h ON g.ГРУППА = h.ГРУППА
INNER JOIN Н_ПЛАНЫ as p ON g.ПЛАН_ИД = p.ИД
INNER JOIN Н_ОТДЕЛЫ as o ON p.ОТД_ИД = o.ИД
WHERE EXTRACT(YEAR FROM g.КОГДА_СОЗДАЛ) = 2011 AND
	o.КОРОТКОЕ_ИМЯ = 'КТиУ'
GROUP BY g.ГРУППА
HAVING COUNT(h.ИД) = 5;
